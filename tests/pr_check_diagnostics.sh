...
GH_DIAGNOSTICS_CALLS="$tmpdir/calls" run_diagnostics normal --failed-diagnostics \
...
run_diagnostics normal --failed-only --failed-diagnostics --quiet \
...
run_diagnostics normal --failed-only --include-failed-logs --quiet \
...
GH_DIAGNOSTICS_CALLS="$tmpdir/collision-calls" run_diagnostics status-collision \
  --failed-diagnostics --quiet >"$tmpdir/status-collision.json"
assert_json '
  (.data.checks | length) == 2 and
...
run_diagnostics status-duplicate --failed-diagnostics --quiet \
...
GH_DIAGNOSTICS_CALLS="$tmpdir/check-run-collision-calls" run_diagnostics check-run-collision \
  --failed-diagnostics --quiet >"$tmpdir/check-run-collision.json"
assert_json '
...
for failures in 0 1 2 10; do
  ...
  GH_DIAGNOSTICS_FAILURES="$failures" \
    ...
    run_diagnostics "$mode" --failed-diagnostics --quiet \
...
if GH_DIAGNOSTICS_FAILURES=2 \
  ...
  run_diagnostics parallel-staggered-error --failed-diagnostics --quiet \
...
if GH_DIAGNOSTICS_FAILURES=4 \
  ...
  run_diagnostics parallel-error --failed-diagnostics --quiet \
...
if GH_DIAGNOSTICS_FAILURES=4 \
  ...
  run_diagnostics parallel-rate-limit --failed-diagnostics --quiet \
...
run_diagnostics pending-metadata --failed-diagnostics --quiet \
...
run_diagnostics normal --include-failed-logs --quiet \
...
run_diagnostics large-log --include-failed-logs --quiet >"$tmpdir/large-log.json"
assert_json '
...
run_diagnostics utf8-boundary --include-failed-logs --quiet >"$tmpdir/utf8-boundary.json"
assert_json '
...
GH_DIAGNOSTICS_REPOSITORY=Owner/Repo run_diagnostics normal \
  --include-failed-logs --quiet >"$tmpdir/mixed-case.json"
assert_json '
  (.data.checks[0].annotations | length) == 3 and
  .data.checks[0].log != null
' "$tmpdir/mixed-case.json" >/dev/null

run_diagnostics required-filter --required --failed-diagnostics --quiet \
...
run_diagnostics required-filter --required --failed-only --failed-diagnostics --quiet \
...
for mode in graphql-error missing-pr; do
  ...
  if run_diagnostics "$mode" --failed-diagnostics --failed-only --quiet \
...
if run_diagnostics non-utf8 --include-failed-logs --quiet \
...
for mode in job-mismatch job-head-mismatch job-link-repository-mismatch job-metadata-repository-mismatch; do
  GH_DIAGNOSTICS_CALLS="$tmpdir/$mode-calls" run_diagnostics "$mode" \
    --include-failed-logs --quiet >"$tmpdir/$mode.json"
...
if run_diagnostics job-id-mismatch --include-failed-logs --quiet \
...
run_diagnostics no-failures --failed-diagnostics \
...
run_diagnostics no-failures --failed-only --failed-diagnostics --quiet \
...
for mode in annotation-failure metadata-failure log-failure; do
  if run_diagnostics "$mode" --include-failed-logs \
    >"$tmpdir/$mode.stdout" 2>"$tmpdir/$mode.stderr"; then
    status=0
  else
    status=$?
...
if run_diagnostics annotation-malformed --failed-diagnostics --quiet \
...
if GH_DIAGNOSTICS_PID_FILE="$tmpdir/pid" run_diagnostics timeout --include-failed-logs --timeout 1 \
...
if run_diagnostics normal --failed-diagnostics --timeout 18446744073709551615 \
...
run_diagnostics normal --failed-diagnostics 2>&- >"$tmpdir/closed-progress.json"
